...
plot_calibrated_figure_3 <- function(){
  ...
  library(readr)
  library(tidyr)
  jared_pgdl <- readr::read_csv('~/Downloads/WRR Figure3 sparse results - Sheet1 (1).csv')  %>% #'fig_3/in/glm_uncal_vs_PGDL_rmses.csv'
    mutate(PGDL_all = as.numeric(`PGDL - full data`), DL_all = as.numeric(`DL - full data`),
