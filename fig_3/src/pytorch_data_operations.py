...
def buildLakeDataForRNN(lakename, seq_length, n_features, train_split=0.2, val_split=0.1, win_shift=1, sparseness=0,
                        flip_trn_test=False, begin_loss_ind=102, specific_train_year=None):
    # PARAMETERS
...
def buildLakeDataForRNN_manylakes_finetunetest(lakename, data_dir, seq_length, n_features, win_shift=1,
                                               begin_loss_ind=102, n_trn_obs=-1, test_seq_per_depth=1,
                                               correlation_check=False, win_per_all=1, win_per_tst=1):
    # PARAMETERS
    # @lakename = string of lake name as the folder of /data/processed/{lakename}
    # @seq_length = sequence length of LSTM inputs
    # @n_features = number of physical drivers
    ...
    assert feat_mat.shape[1] == feat_mat_raw.shape[1]
    win_shift_tst = begin_loss_ind
    win_shift_all = int(seq_length / win_per_all)
    ...
    n_train_seq_no_window = train_seq_per_depth * n_depths
    n_test_seq = (test_seq_per_depth) * n_depths * tst_win_per_seq
    n_all_seq = n_train_seq_no_window * win_per_all
    ...
    tst_dates = np.empty(shape=(n_test_seq, seq_length), dtype='datetime64[s]')
    tst_phys = np.empty(shape=(n_test_seq, seq_length, n_features + 1))  # non-normalized features + ice cover flag
    ...
    for s in range(0, train_seq_per_depth):
        ...
        if end_index > n_dates:
            ...
            n_all_seq -= n_depths
            del_all_seq += 1
            X_all = np.delete(X_all, np.arange(X_all.shape[0], X_all.shape[0] - n_depths * win_per_all, -1), axis=0)
            all_dates = np.delete(all_dates,
                                  np.arange(all_dates.shape[0], all_dates.shape[0] - n_depths * win_per_all, -1),
                                  axis=0)
            X_trn = np.delete(X_trn, np.arange(X_trn.shape[0], X_trn.shape[0] - win_per_seq * n_depths, -1), axis=0)
            continue

        for w in range(0, win_per_all):
            win_start_ind = start_index + w * win_shift_all
            win_end_ind = win_start_ind + seq_length

            for d in range(0, n_depths):
                # first do total model data
                X_all[all_seq_ind, :, :-1] = feat_mat[d, win_start_ind:win_end_ind, :]  # feat
                all_dates[all_seq_ind, :] = dates[win_start_ind:win_end_ind]  # dates
                X_all[all_seq_ind, :, -1] = np.nan  # no label
                X_phys[all_seq_ind, :, :] = feat_mat_raw[d, win_start_ind:win_end_ind, :]
                all_seq_ind += 1
                # now do sliding windows for training data
        for w in range(0, win_per_seq):
            win_start_ind = start_index + w * win_shift
    ...
    if n_test_seq != 0:
        ...
        for s in range(test_seq_per_depth):
            ...
            if end_index > n_dates:
                ...
                X_tst = np.delete(X_tst, np.arange(X_tst.shape[0], X_tst.shape[0] - tst_win_per_seq * n_depths, -1),
                                  axis=0)
                tst_phys = np.delete(tst_phys,
                                     np.arange(tst_phys.shape[0], tst_phys.shape[0] - tst_win_per_seq * n_depths, -1),
                                     axis=0)
                tst_dates = np.delete(X_tst,
                                      np.arange(tst_dates.shape[0], tst_dates.shape[0] - tst_win_per_seq * n_depths,
                                                -1), axis=0)
                continue
            for w in range(0, tst_win_per_seq):
                win_start_ind = start_index + w * win_shift_tst
                ...
                if win_end_ind > n_dates:
                    ...
                    tst_phys = np.delete(tst_phys, -1, axis=0)
                    continue
                for d in range(0, n_depths):
                    X_tst[ts_seq_ind, :, :-1] = feat_mat[d, win_start_ind:win_end_ind, :]
...
def buildLakeDataForRNN_nearbylakes_finetune(lakename, data_dir, seq_length, n_features, win_shift=1,
                                             begin_loss_ind=102, n_trn_obs=-1, test_seq_per_depth=1,
                                             correlation_check=False, win_per_all=1, win_per_tst=1):
    # PARAMETERS
    # @lakename = string of lake name as the folder of /data/processed/{lakename}
    ...
    # @win_shift = days to move in the sliding window for the training set
    # @begin_loss_ind = index in sequence to begin calculating loss function (to avoid poor accuracy in early parts of the sequence)
    # TODO: training windows different that all_windows? training not all depths and all is all depths
    # load data created in preprocess.py based on lakename
    my_path = os.path.abspath(os.path.dirname(__file__))
    ...
    tst = np.load(os.path.join(my_path, "../../data/processed/WRR_69Lake/" + lakename + "/test.npy"))
    trn = np.load(os.path.join(my_path, "../../data/processed/WRR_69Lake/" + lakename + "/train.npy"))
    all_ = np.where(trn == np.nan, tst, trn)
    trn = all_
    # if correlation_check:
    #     #alternative way to divide test/train just by 1/3rd 2/3rd
    #     tst = np.load(os.path.join(my_path, "../../data/processed/WRR_69Lake/"+lakename+"/test_b.npy"))
    ...
    for s in range(0, train_seq_per_depth):
        ...
        if end_index > n_dates:
            n_train_seq -= win_per_seq * n_depths
            n_all_seq -= n_depths * win_per_all
            del_all_seq += 1
            X_all = np.delete(X_all, np.arange(X_all.shape[0], X_all.shape[0] - n_depths * win_per_all, -1), axis=0)
            X_phys = np.delete(X_phys, np.arange(X_phys.shape[0], X_phys.shape[0] - n_depths * win_per_all, -1), axis=0)
            all_dates = np.delete(all_dates,
                                  np.arange(all_dates.shape[0], all_dates.shape[0] - n_depths * win_per_all, -1),
                                  axis=0)
            X_trn = np.delete(X_trn, np.arange(X_trn.shape[0], X_trn.shape[0] - win_per_seq * n_depths, -1), axis=0)
        ...
        for w in range(0, win_per_all):
            win_start_ind = start_index + w * win_shift_all
            win_end_ind = win_start_ind + seq_length
            if (win_end_ind > n_dates) or (feat_mat[0, win_start_ind:win_end_ind, :].shape[0] < seq_length):
                n_all_seq -= n_depths
                X_all = np.delete(X_all, np.arange(X_all.shape[0], X_all.shape[0] - n_depths, -1), axis=0)
                X_phys = np.delete(X_phys, np.arange(X_phys.shape[0], X_phys.shape[0] - n_depths, -1), axis=0)
                all_dates = np.delete(all_dates, np.arange(all_dates.shape[0], all_dates.shape[0] - n_depths, -1),
                                      axis=0)
                continue

            for d in range(0, n_depths):
                # first do total model data
                # print(feat_mat[d,win_start_ind:win_end_ind,:].shape, d)
                X_all[all_seq_ind, :, :-1] = feat_mat[d, win_start_ind:win_end_ind, :]  # feat
                all_dates[all_seq_ind, :] = dates[win_start_ind:win_end_ind]  # dates
                X_all[all_seq_ind, :, -1] = all_[d, win_start_ind:win_end_ind]
                X_phys[all_seq_ind, :, :] = feat_mat_raw[d, win_start_ind:win_end_ind, :]
                all_seq_ind += 1
                # now do sliding windows for training data
        for w in range(0, win_per_seq):
    ...
    if tr_seq_ind != n_train_seq:
        ...
        n_train_seq -= extra
        X_trn = np.delete(X_trn, np.arange(X_trn.shape[0], X_trn.shape[0] - extra, -1), axis=0)
    if X_all.shape[0] != n_all_seq:
        extra = X_all.shape[0] - n_all_seq + 1
        # n_all_seq -= extra
        X_all = np.delete(X_all, np.arange(X_all.shape[0], X_all.shape[0] - extra, -1), axis=0)
        all_dates = np.delete(all_dates, np.arange(all_dates.shape[0], all_dates.shape[0] - extra, -1), axis=0)
        X_phys = np.delete(X_phys, np.arange(X_phys.shape[0], X_phys.shape[0] - extra, -1), axis=0)
    ...
    # assert ts_seq_ind == n_test_seq, \
    #     "incorrect number of tst seq estimated {} vs actual{}".format(n_test_seq, ts_seq_ind)
    assert X_all.shape[0] == n_all_seq, \
        "incorrect number of all seq tracked {} vs actual{}".format(n_all_seq,
                                                                    X_all.shape[0])  # remove sequences with no labels
    ...
    tst_del_ind = np.array([], dtype=np.int32)
    all_seq_removed = 0
    all_del_ind = np.array([], dtype=np.int32)
    all_del_ind2 = np.array([], dtype=np.int32)

    for i in range(n_train_seq):
        # remove sequence if there are no values in it
        if np.isfinite(X_trn[i, begin_loss_ind:, -1]).any():
            continue
        else:
            # print(X_trn[i,:,-1])
    ...
    # for i in range(n_all_seq):
    # this isn't needed because we want to keep them all for the all
    #     if np.isfinite(X_all[i,begin_loss_ind:,:-1]).all():
    #         continue
    #     else:
    #         # print(X_trn[i,:,-1])
    #         all_seq_removed += 1
    #         all_del_ind = np.append(all_del_ind, i)

    X_all_tmp = np.delete(X_all, all_del_ind, axis=0)
    all_dates_tmp = np.delete(all_dates, all_del_ind, axis=0)
    X_phys_tmp = np.delete(X_phys, all_del_ind, axis=0)
    X_all = X_all_tmp
    X_phys = X_phys_tmp
    all_dates = all_dates_tmp

    # for i in range(n_all_seq):
    #     if np.isfinite(X_all[i,begin_loss_ind:,-1]).any():
    #         continue
    #     else:
    #         all_del_ind2 = np.append(all_del_ind2, i)
    #         all_seq_removed += 1

    X_all_tmp = np.delete(X_all, all_del_ind2, axis=0)
    all_dates_tmp = np.delete(all_dates, all_del_ind2, axis=0)
    X_phys_tmp = np.delete(X_phys, all_del_ind2, axis=0)
    X_all = X_all_tmp
    X_phys = X_phys_tmp
    all_dates = all_dates_tmp
...
def buildLakeDataForRNN_manylakes_finetune(lakename, data_dir, seq_length, n_features, win_shift=1, begin_loss_ind=102,
                                           n_trn_obs=-1, test_seq_per_depth=1, correlation_check=False,
    ...
    # if not allTestSeq: #if we don't want to output ALL
    if outputFullTestMatrix:
        # if we want to output the full test matrix
        tempy = np.nan  # do nothing
    else:
        for i in range(n_test_seq):
            if np.isfinite(X_tst[i, begin_loss_ind:, -1]).any():
                continue
...
def buildLakeDataForRNN_manylakes_finetune2(lakename, data_dir, seq_length, n_features, win_shift=1, begin_loss_ind=100,
                                            n_trn_obs=-1, test_seq_per_depth=1, correlation_check=False,
                                            outputFullTestMatrix=False, sparseTen=False, sparse50=False, realization=0,
                                            allTestSeq=False, targetLake=None):
    # PARAMETERS
    # @lakename = string of lake name as the folder of /data/processed/{lakename}
    # @seq_length = sequence length of LSTM inputs
    # @n_features = number of physical drivers
    ...
    if sparseTen:
        trn = np.load(
            os.path.join(my_path, "../../data/processed/WRR_69Lake/" + lakename + "/train10_" + realization + ".npy"))
    if sparse50:
        trn = np.load(
            os.path.join(my_path, "../../data/processed/WRR_69Lake/" + lakename + "/train50_" + realization + ".npy"))
    ...
    for s in range(0, train_seq_per_depth):
        ...
        if end_index > n_dates:
            ...
            X_all = np.delete(X_all, np.arange(X_all.shape[0], X_all.shape[0] - n_depths, -1), axis=0)
            X_trn = np.delete(X_trn, np.arange(X_trn.shape[0], X_trn.shape[0] - win_per_seq * n_depths, -1), axis=0)
            trn_dates = np.delete(trn_dates,
                                  np.arange(trn_dates.shape[0], trn_dates.shape[0] - n_depths * win_per_seq, -1),
                                  axis=0)
            continue
        for d in range(0, n_depths):
            # first do total model data
        ...
        for s in range(test_seq_per_depth):
            ...
            for w in range(0, tst_win_per_seq):
                ...
                if win_end_ind > n_dates:
                    ...
                    tst_dates = np.delete(tst_dates, np.arange(tst_dates.shape[0], tst_dates.shape[0] - n_depths, -1),
                                          axis=0)
                    continue
                for d in range(0, n_depths):
                    X_tst[ts_seq_ind, :, :-1] = feat_mat[d, win_start_ind:win_end_ind, :]
                    ...
                    tst_dates[ts_seq_ind, :] = dates[win_start_ind:win_end_ind]
                    ts_seq_ind += 1
    while tst_dates[-1, 0] == np.datetime64("NaT"):
        tst_dates = np.delete(tst_dates, -1, axis=0)
        X_tst = np.delete(X_tst, -1, axis=0)
    ...
    if allTestSeq:
        X_tst = np.vstack((X_trn, X_tst))
        tst_dates = np.vstack((trn_dates, tst_dates))
    ...
    # assert np.isfinite(all_dates).any(), "all_dates has nan"
    return (
    torch.from_numpy(X_trn), trn_dates, torch.from_numpy(X_tst), tst_dates, unique_tst_dates, torch.from_numpy(X_all),
    torch.from_numpy(X_phys), all_dates, hyps)


def buildLakeDataForRNN_manylakes_finetune_singledepth(lakename, data_dir, seq_length, n_features, depth_ind,
                                                       win_shift=1, begin_loss_ind=102, n_trn_obs=-1,
                                                       test_seq_per_depth=1, correlation_check=False):
    # PARAMETERS
    # @lakename = string of lake name as the folder of /data/processed/{lakename}
    # @seq_length = sequence length of LSTM inputs
    # @n_features = number of physical drivers
...
def buildLakeDataForRNN_manylakes_finetune_returnTest(lakename, data_dir, seq_length, n_features, win_shift=1,
                                                      begin_loss_ind=102, n_trn_obs=-1, test_seq_per_depth=1,
    ...
    test_seq_per_depth = seq_per_depth
    win_per_seq = math.floor(seq_length / win_shift) - 1  # windows per sequence (only training)
    tst_win_per_seq = 1  # windows per sequence (only testing)
    n_train_seq = train_seq_per_depth * n_depths * win_per_seq
    n_train_seq_no_window = train_seq_per_depth * n_depths
    n_test_seq = (test_seq_per_depth) * n_depths * tst_win_per_seq
        ...
        for s in range(test_seq_per_depth):
            ...
            if end_index > n_dates:
                ...
                tst_phys = np.delete(tst_phys,
                                     ...
                                     axis=0)
                tst_dates = np.delete(tst_dates,
                                      np.arange(tst_dates.shape[0], tst_dates.shape[0] - tst_win_per_seq, -1), axis=0)
            ...
            for w in range(0, tst_win_per_seq):
                ...
                for d in range(0, n_depths):
                    X_tst[ts_seq_ind, :, :-1] = feat_mat[d, win_start_ind:win_end_ind, :]
                    X_tst[ts_seq_ind, :, -1] = tst[d, win_start_ind:win_end_ind]
                    tst_phys[ts_seq_ind, :, :] = feat_mat_raw[d, win_start_ind:win_end_ind, :]
                    tst_dates[ts_seq_ind, :] = dates[win_start_ind:win_end_ind]
                    ts_seq_ind += 1
    ...
    for i in range(n_test_seq):
        # delete if not any observations in period
        if np.isfinite(X_tst[i, begin_loss_ind:, -1]).any():
            continue
        else:
            tst_del_ind = np.append(tst_del_ind, i)
    ...
    X_tst_tmp = np.delete(X_tst, tst_del_ind, axis=0)
    tst_dates_tmp = np.delete(tst_dates, tst_del_ind, axis=0)
    tst_phys_tmp = np.delete(tst_phys, tst_del_ind, axis=0)
    X_trn = X_trn_tmp
    X_tst = X_tst_tmp
    tst_dates = tst_dates_tmp
    tst_phys = tst_phys_tmp
    ...
    # assert np.isfinite(all_dates).any(), "all_dates has nan"
    return (torch.from_numpy(X_trn), torch.from_numpy(X_tst), torch.from_numpy(X_all),
            torch.from_numpy(X_phys), all_dates, hyps, tst_dates, torch.from_numpy(tst_phys))


def buildLakeDataForRNN_manylakes_finetune_returnTestExtend(lakename, data_dir, seq_length, n_features, win_shift=1,
                                                            begin_loss_ind=102, n_trn_obs=-1, test_seq_per_depth=1,
                                                            correlation_check=False, tst_win_shift=100,
                                                            all_win_per_seq=2):
    # PARAMETERS
    # @lakename = string of lake name as the folder of /data/processed/{lakename}
    # @seq_length = sequence length of LSTM inputs
    ...
    assert feat_mat.shape[1] == trn.shape[1]
    assert feat_mat.shape[1] == feat_mat_raw.shape[1]
    win_shift_tst = tst_win_shift
    win_shift_all = int(seq_length / all_win_per_seq)
    depth_values = feat_mat_raw[:, 0, 0]
    assert np.unique(depth_values).size == n_depths
    udates = dates
    n_dates = feat_mat.shape[1]
    ...
    test_seq_per_depth = seq_per_depth
    win_per_seq = math.floor(seq_length / win_shift) - 1  # windows per sequence (only training)
    tst_win_per_seq = 2  # windows per sequence (only testing)
    n_train_seq = train_seq_per_depth * n_depths * win_per_seq
    n_train_seq_no_window = train_seq_per_depth * n_depths
    n_test_seq = (test_seq_per_depth) * n_depths * tst_win_per_seq

    n_all_seq = n_train_seq_no_window * all_win_per_seq
    ...
    X_all = np.empty(shape=(n_all_seq, seq_length, n_features))  # features
    all_dates = np.empty(shape=(n_all_seq, seq_length), dtype='datetime64[s]')
    X_phys = np.empty(shape=(n_all_seq, seq_length, n_features + 1))  # non-normalized features + ice cover flag
    ...
    for s in range(0, train_seq_per_depth):
        ...
        if end_index > n_dates:
            n_train_seq -= win_per_seq * n_depths
            n_all_seq -= all_win_per_seq * n_depths
            del_all_seq += 1
            X_all = np.delete(X_all, np.arange(X_all.shape[0], X_all.shape[0] - all_win_per_seq * n_depths, -1), axis=0)
            X_phys = np.delete(X_phys, np.arange(X_phys.shape[0], X_phys.shape[0] - all_win_per_seq * n_depths, -1),
                               axis=0)
            all_dates = np.delete(all_dates,
                                  np.arange(all_dates.shape[0], all_dates.shape[0] - all_win_per_seq * n_depths, -1),
                                  axis=0)
            X_trn = np.delete(X_trn, np.arange(X_trn.shape[0], X_trn.shape[0] - win_per_seq * n_depths, -1), axis=0)
            continue
        # for d in range(0, n_depths):
        ...
        #     X_phys[all_seq_ind, :, :] = feat_mat_raw[d, start_index:end_index,:]
        #     all_seq_ind += 1
        for w in range(0, all_win_per_seq):
            # create X_all containing all data
            win_start_ind = start_index + w * win_shift_all
            win_end_ind = win_start_ind + seq_length
            if win_end_ind > n_dates:
                # dont add incomplete sequences
                n_all_seq -= 1
                X_all = np.delete(X_all, -1, axis=0)
                all_dates = np.delete(all_dates, -1, axis=0)
                X_phys = np.delete(X_phys, -1, axis=0)

                continue
            for d in range(0, n_depths):
                X_all[all_seq_ind, :, :] = feat_mat[d, win_start_ind:win_end_ind, :]
                all_dates[all_seq_ind, :] = dates[win_start_ind:win_end_ind]  # dates
                X_phys[all_seq_ind, :, :] = feat_mat_raw[d, win_start_ind:win_end_ind, :]
                all_seq_ind += 1
        for w in range(0, win_per_seq):
    ...
    if all_seq_ind != n_all_seq:
        print("mismatch estimated vs actual all sequences")
        extra = n_all_seq - all_seq_ind
        n_all_seq -= extra
        X_all = np.delete(X_all, np.arange(X_all.shape[0], X_all.shape[0] - extra, -1), axis=0)
        all_dates = np.delete(all_dates, np.arange(all_dates.shape[0], all_dates.shape[0] - extra, -1), axis=0)
        X_phys = np.delete(X_phys, np.arange(X_phys.shape[0], X_phys.shape[0] - extra, -1), axis=0)

    while ~np.isfinite(X_all[:, :, :-1]).all():
        X_all = np.delete(X_all, -1, axis=0)
        all_dates = np.delete(all_dates, -1, axis=0)
        X_phys = np.delete(X_phys, -1, axis=0)
    ...
    # assert data was constructed correctly
    if n_test_seq > ts_seq_ind:
        X_tst = np.delete(X_tst, np.arange(X_tst.shape[0], X_tst.shape[0] - (n_test_seq - ts_seq_ind), -1), axis=0)
        tst_phys = np.delete(tst_phys, np.arange(tst_phys.shape[0], tst_phys.shape[0] - (n_test_seq - ts_seq_ind), -1),
                             axis=0)
        tst_dates = np.delete(tst_dates,
                              np.arange(tst_dates.shape[0], tst_dates.shape[0] - (n_test_seq - ts_seq_ind), -1), axis=0)
        n_test_seq = ts_seq_ind
...
def buildLakeDataForRNNPretrain(lakename, data_dir, seq_length, n_features, win_shift=1, begin_loss_ind=102,
                                excludeTest=False, targetLake=None):
    # PARAMETERS
    # @lakename = string of lake name as the folder of /data/processed/{lakename}
    # @seq_length = sequence length of LSTM inputs
    # @n_features = number of physical drivers
    ...
    assert feat_mat.shape[0] == feat_mat_raw.shape[0]
    assert feat_mat.shape[0] == Y_mat.shape[0]
    assert feat_mat.shape[1] == Y_mat.shape[1], "feat mat shape[1]=" + str(
        feat_mat.shape[1]) + " vs Y_mat.shape[1]=" + str(Y_mat.shape[1])
    assert feat_mat.shape[1] == feat_mat_raw.shape[1]
    depth_values = feat_mat_raw[:, 0, 0]
    assert np.unique(depth_values).size == n_depths
    udates = dates
...
def buildLakeDataForRNNPretrainSingleDepth(lakename, data_dir, seq_length, n_features, depth_ind, win_shift=1,
                                           begin_loss_ind=102):
    # PARAMETERS
    # @lakename = string of lake name as the folder of /data/processed/{lakename}
    # @seq_length = sequence length of LSTM inputs
...
def plot_ec_loss(lakename, all_data, all_phys_data, all_dates, hypsography, n_depths, depth_areas, combine_days=1):
    ...
    for s in range(n_sets):
        ...
        # print(doy)
        lake_energies = calculate_lake_energy(y[start_index:end_index, :], densities[start_index:end_index, :],
                                              depth_areas)
    ...
    ax.set(xlabel="doy", ylabel='W/m^2 diff',
           title='Average Absolute Difference between sum of fluxes and lake energy change for GLM on ' + lakename)
    plt.legend()
    plt.show()
...
def calculate_ec_loss_manylakes(inputs, outputs, phys, labels, dates, depth_areas, n_depths, ec_threshold, use_gpu,
                                combine_days=1):
    import numpy as np
    # ******************************************************
...
def calculate_ec_loss(inputs, outputs, phys, labels, dates, depth_areas, n_depths, ec_threshold, use_gpu,
    ...
    for i in range(n_sets):
        ...
        ### can use this to plot energy delta and flux over time to see if they line up
        doy = np.array([datetime.datetime.combine(date.fromordinal(x), datetime.time.min).timetuple().tm_yday for x in
                        dates[start_index, :]])
        doy = doy[1:-1]
        diff_vec = (lake_energy_deltas - lake_energy_fluxes).abs_()
...
def getHypsography(lakename, depths, debug=False, path=None):
    ...
    if path is not None:
        ...
        depth_areas = pd.read_csv(
            os.path.join(my_path, '../../data/raw/' + lakename + '/' + lakename + '_hypsography.csv'), header=0,
...
def parseMatricesFromSeqs(pred, targ, depths, dates, n_depths, n_tst_dates, u_depths, u_dates):
    ...
    for i in range(n_seq):
        ...
        # find depth index
        depth_ind = np.where(abs(u_depths - depths[i, 0].item()) <= .000001)[0][0]
