# https://github.com/jdwillard19/lake_modeling/blob/a2fb7239c929e14f6647faa21167e96ad79c0cef/src/data/preprocess_functions.py
...
def preprocess(target_id, addDirRemove=0):
    ...
    #############################################
    meteo_dates = np.loadtxt(addStr + '../../data/raw/figure3/nhd_' + name + '_meteo.csv', delimiter=',',
                             dtype=np.string_, usecols=0)
    ...
    meteo = np.genfromtxt(addStr + '../../data/raw/figure3/nhd_' + name + '_meteo.csv', delimiter=',',
    ...
    for filename in os.listdir(directory):
        ...
        if name2 not in o_names:
            ...
            # read/format meteorological data for numpy
            #############################################
            meteo_dates = np.loadtxt(addStr + '../../data/raw/figure3/nhd_' + name2 + '_meteo.csv', delimiter=',',
                                     dtype=np.string_, usecols=0)
            ...
            if len(np.where(meteo_dates == end_date)[0]) < 1:
                print(
                    "observation beyond meteorological data! data will only be used up to the end of meteorological data")
                upper_cutoff = meteo_dates.shape[0]
            else:
                upper_cutoff = np.where(meteo_dates == end_date)[0][0] + 1  # 14233
            ...
            # read from file and filter dates
            meteo = np.genfromtxt(addStr + '../../data/raw/figure3/nhd_' + name2 + '_meteo.csv', delimiter=',',
                                  usecols=(1, 2, 3, 4, 5, 6, 7))
            ...
            if len(np.where(glm_temps_loi[:, 0] == start_date_loi.decode())[0]) < 1:
                print(
                    "loi observations begin at " + start_date_loi.decode() + "which is before GLM data which begins at " +
                    glm_temps_loi[0, 0])
                lower_cutoff_loi = 0
            ...
            if len(np.where(glm_temps_loi[:, 0] == end_date_loi.decode())[0]) < 1:
                print("observations extend to " + end_date_loi.decode() + "which is beyond GLM data which extends to " +
                      glm_temps_loi[-1, 0])
            ...
            feat_norm_mat = np.empty(
                (n_depths, n_dates, n_features + 1))  # [standardized depth -> 7 std meteo features]
