...
label = np.load('labels_mendota.npy')
mask = 1-np.isnan(label)
label[np.isnan(label)]=0

m_tr = mask
m_te = mask
#m_tr = np.ones([label.shape[0],label.shape[1]])*1.0
#m_te = np.ones([label.shape[0],label.shape[1]])*1.0
...
for i in range(1,N_sec+1):
    ...
    y_train[(i-1)*50:i*50,:]=label[:,int((i-1)*n_steps/2):int((i+1)*n_steps/2)]
    ...
    y_test[(i-1)*50:i*50,:]=label[:,int((i-1)*n_steps/2):int((i+1)*n_steps/2)]
...
#        for i in range(total_batch - 1): # better code?
        ...
        if epoch>30:
            ...
            if merr>loss_te:
                ...
                metr= rc
                if epoch>90:
                    save_path = saver.save(sess, "./pretrained_model/model_mendota"+str(epoch)+"_b.ckpt")
