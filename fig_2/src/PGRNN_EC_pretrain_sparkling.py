...
label = np.load('labels_sparkling.npy')
...
for i in range(1,N_sec+1):
    ...
    y_train[(i-1)*37:i*37,:]=label[:,int((i-1)*n_steps/2):int((i+1)*n_steps/2)]
    ...
    y_test[(i-1)*37:i*37,:]=label[:,int((i-1)*n_steps/2):int((i+1)*n_steps/2)]
...
#        for i in range(total_batch - 1): # better code?
        ...
        if epoch>30:
            ...
            if merr>loss_te:
                ...
                metr= rc
                if epoch>90:
                    save_path = saver.save(sess, "./pretrained_model/model_sparkling"+str(epoch)+"_b.ckpt")
