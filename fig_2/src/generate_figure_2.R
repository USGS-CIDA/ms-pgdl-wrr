plot_seasons_years_sparsity <- function(){
  ...
  par(omi = c(0,0.4,0.05,0.25), mai = c(0.5,0.2,0,0), las = 1, mgp = c(2.3,.5,0), cex = 1.5)
  layout(mat = matrix(c(1,2,3), nrow = 1))

  set_plot <- function(text, panel){
    plot(NA, NA, xlim = c(0.7, 3.3), ylim = c(3.3, .95),
         ylab = '', xlab = "", axes = FALSE, xaxs = 'i', yaxs = 'i')
    if (panel == 'a)'){
      axis(2, at = seq(0,10, by = 0.5), las = 1, tck = -0.01)
      mtext(text = 'Test RMSE (°C)', side = 2, las = 3, outer = 3, cex = 0.9, padj = -1.5)
    } else {
    ...
    par(mgp = c(2.3,0.5,0))
    text(x = 0.75, y = 1.03, panel, font = 2, pos = 4, cex = 1.3)
    if (length(text) == 1){
      text(x = 0.75, y = 1.16, text, pos = 4, cex = 1.1)
    } else {
      text(x = 0.75, y = 1.16, text[1], pos = 4, cex = 1.1)
      text(x = 0.75, y = 1.26, text[2], pos = 4, cex = 1.1)
    }
  ...
  plot_all_models <- function(plot_data){
    ...
    points(RNN_x+me_bmp, filter(plot_data, lake == "mendota", Model == 'RNN') %>% pull(`Test RMSE`) %>% mean, pch = 22, lwd = 1.5, cex = 3.5, bg = 'white', col = '#d95f02')
    ...
    ME_mean <- filter(plot_data, lake == "mendota", Model == 'PGRNN') %>% pull(`Test RMSE`) %>% mean
    lines(c(PGRNN_x+me_bmp, PGRNN_x+me_bmp), c(filter(plot_data, lake == "mendota", Model == 'PGRNN') %>% pull(`Test RMSE`) %>% max(),
                                               filter(plot_data, lake == "mendota", Model == 'PGRNN') %>% pull(`Test RMSE`) %>% min()), lwd = 1.5, col = '#7570b3')
    ...
    points(GLM_x+me_bmp, ME_mean, pch = 21, lwd = 1.5, cex = 3.2, bg = 'white', col = '#1b9e77')
    ...
    points(RNN_x+sp_bmp, SP_mean, pch = 22, lwd = 1.5, cex = 3.5, bg = 'white', col = '#d95f02')
    text(RNN_x+sp_bmp, SP_mean, "S", font = 2)


    SP_mean <- filter(plot_data, lake == "sparkling", Model == 'PGRNN') %>% pull(`Test RMSE`) %>% mean
    lines(c(PGRNN_x+sp_bmp, PGRNN_x+sp_bmp), c(filter(plot_data, lake == "sparkling", Model == 'PGRNN') %>% pull(`Test RMSE`) %>% max(),
    ...
    points(GLM_x+sp_bmp, SP_mean, pch = 21, lwd = 1.5, cex = 3.2, bg = 'white', col = '#1b9e77')
