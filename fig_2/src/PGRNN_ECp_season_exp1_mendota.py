...
# load data ---------------------------------------------------------------
x_full = np.load('processed_features_mendota.npy')
x_raw_full = np.load('features_mendota.npy')
diag_full = np.load('diag_mendota.npy')

phy_full = np.concatenate((x_raw_full[:,:,:-2],diag_full),axis=2)


# training and testing ------------------------------------------------------
new_dates = np.load('dates_mendota.npy')

train_data = feather.read_dataframe('../data/Trainingdata/mendota_season_training_500_profiles_experiment_01.feather')
#train_data.columns
...
test_data = feather.read_dataframe('../data/Testdata/mendota_season_test.feather')
#test_data.columns
...
with tf.Session() as sess:
    ...
    if using_pretrained==1:
        saver.restore(sess, "./pretrained_model/model_mendota449_b.ckpt")
...
#               
                ...
                for j in range(N_sec-1):   # 18*125    +250 = 2500
                    ...
                    prd_o[:, 353+int(j*353/2):353+int((j+1)*353/2)] = prd[(j+1)*50:(j+2)*50,st_idx:,0]
                
                # calculate RMSE
                ttt = np.sum(m_te)
                rm = np.sqrt(np.sum(np.power((prd_o-obs_te)*m_te,2))/ttt)
                np.save('PGRNN_season_mendota_exp1.npy',prd_o)
